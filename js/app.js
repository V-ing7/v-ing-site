...
(function(){
  ...
  window.addEventListener('ving-lang-changed', function(){
    ...
    if(typeof refreshCqtUI === 'function') refreshCqtUI();
  });

  // ---------- Creator Quick Tools (multi-select open) ----------
  function initCreatorQuickTools(){
    var grid = document.getElementById('cqtGrid');
    var selectAllBtn = document.getElementById('cqtSelectAll');
    var openBtn = document.getElementById('cqtOpenSelected');
    if(!grid) return;

    window.refreshCqtUI = function(){
      var checks = grid.querySelectorAll('.cqt-check');
      var checked = grid.querySelectorAll('.cqt-check:checked');
      var n = checked.length;
      var total = checks.length;
      var lang = (localStorage.getItem('v-ing-lang') || 'zh');
      if(openBtn){
        if(lang === 'zh'){
          openBtn.textContent = n > 0 ? '打开选中 (' + n + ')' : '打开选中';
        } else {
          openBtn.textContent = n > 0 ? 'Open Selected (' + n + ')' : 'Open Selected';
        }
        openBtn.disabled = n === 0;
      }
      checks.forEach(function(cb){
        var card = cb.closest('.cqt-btn');
        if(card) card.classList.toggle('selected', cb.checked);
      });
      if(selectAllBtn){
        var allOn = (n === total && total > 0);
        if(lang === 'zh'){
    ...
    grid.addEventListener('click', function(e){
      var check = e.target.closest('.cqt-check');
      var card = e.target.closest('.cqt-btn');
      if(check){ refreshCqtUI(); return; }
      if(card){
  ...
  if(document.readyState === 'loading'){
    document.addEventListener('DOMContentLoaded', initCreatorQuickTools);
  } else {
    initCreatorQuickTools();
  }
