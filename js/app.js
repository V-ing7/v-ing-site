...
// ====== Storyboard: Collapsible Sections + Script ======
var __sbScriptInitialized = false;

function initStoryboardScript(){
  if(__sbScriptInitialized) return;
  __sbScriptInitialized = true;
  
  // Add CSS (in case stylesheet hasn't loaded)
  // Styles are in style.css, but we ensure layout works
  
  // Make all section headers collapsible
  setupCollapsibleSections();
  
  // Add script section to editor
  addScriptSectionToEditor();
  
  // Add script section to detail view  
  addScriptSectionToDetail();
}
...
function setupCollapsibleSections(){
  ...
  var editorView = document.getElementById('sbEditorView');
  if(editorView){
    makeSectionsCollapsible(editorView);
  }
  // Detail view
  var detailWrap = document.getElementById('sbDetailView') || findDetailView();
  if(detailWrap){
    makeSectionsCollapsible(detailWrap);
  }
}

function findDetailView(){
  // Try to find detail view in storyboard panel
  var panel = document.getElementById('ws-panel-storyboard');
  if(panel){
    var views = panel.querySelectorAll('.sb-wrap');
    for(var i=0;i<views.length;i++){
      if(views[i].id !== 'sbEditorView' && views[i].id !== 'sbProjectListView'){
        return views[i];
      }
    }
  }
...
function makeSectionsCollapsible(container){
  var sections = container.querySelectorAll('.sb-section');
  for(var i=0;i<sections.length;i++){
    var sec = sections[i];
    var header = sec.querySelector('.sb-section-header');
    if(!header) continue;
    
    // Add collapsible class if not present
    if(!header.classList.contains('sb-collapsible')){
      header.classList.add('sb-collapsible');
      header.addEventListener('click', function(e){
        var s = this.closest('.sb-section');
        if(s) s.classList.toggle('collapsed');
      });
      
      // Add arrow
      var arrow = document.createElement('span');
      arrow.className = 'sb-collapse-arrow';
      arrow.textContent = '▼';
    ...
    if(next && !next.classList.contains('sb-section-content')){
      var wrap = document.createElement('div');
      wrap.className = 'sb-section-content';
      sec.insertBefore(wrap, next);
      wrap.appendChild(next);
    }
  }
}

function addScriptSectionToEditor(){
  var editorView = document.getElementById('sbEditorView');
  if(!editorView) return;
  if(document.getElementById('sbScriptList')) return;
  
  var sectionsContainer = editorView.querySelector('.sb-sections');
  if(!sectionsContainer) return;
  
  var scriptSection = document.createElement('div');
  scriptSection.className = 'sb-section sb-section-script';
  scriptSection.innerHTML =
    '<div class="sb-section-header sb-collapsible">' +
    '<span class="sb-section-dot sb-section-dot-script"></span>' +
    '<span class="sb-section-title" data-cn="口播稿" data-en="Script">口播稿</span>' +
    '<span class="sb-collapse-arrow">▼</span>' +
    '</div>' +
    '<div class="sb-section-content">' +
    '<div class="sb-script-list" id="sbScriptList"></div>' +
    '</div>';
  
  sectionsContainer.appendChild(scriptSection);
  
  // Add click handler
  var header = scriptSection.querySelector('.sb-section-header');
  header.addEventListener('click', function(){
    scriptSection.classList.toggle('collapsed');
  });
}

function addScriptSectionToDetail(){
  var detailWrap = findDetailView();
  if(!detailWrap) return;
  if(document.getElementById('sbDetailScriptList')) return;
  
  var sectionsContainer = detailWrap.querySelector('.sb-sections');
  if(!sectionsContainer) return;
  ...
  scriptSection.innerHTML =
    ...
    '</div>' +
    '<div class="sb-section-content">' +
    '<div class="sb-script-list" id="sbDetailScriptList"></div>' +
    '</div>';
...
function renderScriptList(){
  var scriptList = document.getElementById('sbScriptList');
  if(!scriptList) return;
  
  scriptList.innerHTML = '';
  if(typeof editRows === 'undefined') return;
  
  editRows.forEach(function(row, idx){
    var item = document.createElement('div');
    item.className = 'sb-script-item';
    item.innerHTML = '<textarea data-idx="' + idx + '" placeholder="输入口播稿..." rows="2">' + escapeHtml(row.script || '') + '</textarea>';
    ...
    ta.addEventListener('input', function(){
      ta.style.height = 'auto';
      ta.style.height = ta.scrollHeight + 'px';
      if(typeof editRows !== 'undefined'){
        editRows[idx].script = ta.value;
      }
...
function renderDetailScriptList(rows){
  var list = document.getElementById('sbDetailScriptList');
  if(!list) return;
  
  list.innerHTML = '';
  
  if(!rows || rows.length === 0){
    list.innerHTML = '<div style="text-align:center;padding:30px;color:var(--text-quaternary)">' + (typeof t === 'function' ? t('暂无口播稿','No script') : '暂无口播稿') + '</div>';
    return;
  }
  
  rows.forEach(function(row, idx){
    var item = document.createElement('div');
    item.className = 'sb-detail-script-item';
    item.innerHTML =
      '<div class="sb-detail-script-num">' + (idx+1) + '</div>' +
      '<div class="sb-detail-script-text" style="white-space:pre-wrap;word-break:break-word">' + escapeHtml(row.script || '') + '</div>';
    list.appendChild(item);
  });
}

// Patch existing functions
// - When opening editor, also render script list
// - When saving, include script field
// - When adding row, add to script list too

(function(){
  // Patch renderTaskCards to also call renderScriptList
  var origRender = null;
  var checkInterval = setInterval(function(){
    if(typeof renderTaskCards === 'function' && !origRender){
      origRender = renderTaskCards;
      window.renderTaskCards = function(){
        origRender();
        // Initialize script sections after first render
        initStoryboardScript();
        renderScriptList();
      };
      clearInterval(checkInterval);
    }
  ...
  // Also patch save function
  var saveCheck = setInterval(function(){
    if(typeof saveStoryboard === 'function' || typeof sbSave === 'function'){
      // The save function should already use editRows which now has script field
      clearInterval(saveCheck);
    }
  }, 100);
  
  // Patch detail view render
  var detailCheck = setInterval(function(){
    if(typeof showProjectDetail === 'function' || typeof renderProjectDetail === 'function'){
      var origDetail = typeof showProjectDetail === 'function' ? showProjectDetail : renderProjectDetail;
      var funcName = typeof showProjectDetail === 'function' ? 'showProjectDetail' : 'renderProjectDetail';
      window[funcName] = function(){
        var result = origDetail.apply(this, arguments);
        setTimeout(function(){
          initStoryboardScript();
          // Try to get rows from current project
          if(window.__vingData && window.__vingData.storyboardProjects){
            var detailView = document.querySelector('.sb-wrap:not(#sbEditorView):not(#sbProjectListView)');
            if(detailView){
              var pid = detailView.getAttribute('data-pid');
              if(pid){
                var proj = window.__vingData.storyboardProjects.find(function(p){ return p.id === pid; });
                if(proj){
                  renderDetailScriptList(proj.rows);
                }
              }
            }
          }
...
if(document.readyState === 'loading'){
  document.addEventListener('DOMContentLoaded', function(){
    setTimeout(initStoryboardScript, 500);
  });
} else {
  setTimeout(initStoryboardScript, 500);
}
